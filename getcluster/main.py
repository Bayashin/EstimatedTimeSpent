...
def main():
    ...
    for user in user_list:
        ...
        for i in range(7):
            ...
            df_entry: list[str] = df_day['reporting'].to_list()
            df_exit: list[str] = df_day['leave'].to_list()
            # 時間データを秒単位に変換
            data_seconds_entry: list[int] = [sum(x * int(t) for x, t in zip([3600, 60, 1], point.split(":"))) for point in df_entry]
            data_seconds_exit: list[int] = [sum(x * int(t) for x, t in zip([3600, 60, 1], point.split(":"))) for point in df_exit]
            # クラスタリングの実行
            # 結果をリストに格納
